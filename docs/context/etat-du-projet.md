...
- **`medical_entities`** (2177 lignes) : colonnes OSM d'origine (legacy, lecture seule) + taxonomie (`type_slug` FK→medical_types, `category_slug`, `classification_status`) + géo en **codes officiels INSTAT** : `code_faritra` CHAR(2), `code_distrika` CHAR(4), `code_kaominina` CHAR(6), `code_fokontany` CHAR(8) FK→fokontany (profondeur variable selon géocodage). `owner_id` reste varchar (pas encore FK, voir dette A-2).
- **Taxonomie** : `medical_categories` (6) + `medical_types` (25), libellés fr/mg/en. Slugs catégories : urgences_secours, soins_proximite, soins_specialises, maternite_enfance, produits_sante, diagnostic_analyse.
- **Géographie** : table **`fokontany` dénormalisée** (structure data-personne) : **19 336 fokontany, 1704 communes, 119 districts, 23 régions** avec codes hiérarchiques officiels + noms aux 4 niveaux. Copiée de la base `datapersonne`. Reproduction : `backend/scripts/README-geo.md`.
- **Classification** : ~656 `osm_auto`, ~1521 `unverified`. **Géocodage** des entités : faritra 94%, distrika 91%, kaominina 63%, fokontany 52% (best-effort).
...
- **Endpoints réorg** : `GET /api/taxonomy` (arbre cat+types), `/api/taxonomy/types`, `/api/geo/faritra`, `/api/geo/distrika?faritra=CODE`, `/api/geo/kaominina?distrika=CODE` (renvoient `{code, nom}`). `/api/places` accepte `category`, `type` (slug + repli OSM), `faritra`, `distrika`, `kaominina` (codes), `status`. `/api/places/stats` renvoie `byCategory`, `byType`, `unverified`.
